...
**Emotional goals:** Satisfaction and reward from tracking progress. Excitement and motivation from seeing stats, milestones, and the growing collection. The interface should make stitching *more* rewarding, not add busywork.
...
- **Typography:** Fraunces (serif) for headings brings character and warmth. Source Sans 3 (sans) for body keeps things clean and readable. JetBrains Mono reserved *only* for hero stat numbers and progress percentages — never in tables or body text.
...
- **Whimsy moments:** Achievement badges, progress milestones, empty state illustrations, subtle animations on stat changes. These are *accents*, not the default tone.
...
- Color is never the *only* indicator — status badges include text labels alongside colored dots
