...
describe("discardStoredObjects", () => {
  ...
  /** The batch-delete requests issued, as bucket plus the keys each carried. */
  function deleteBatches(): { Bucket: string; Keys: string[] }[] {
    return mockSend.mock.calls
      .map(
        ([command]) => command as { constructor: { name: string }; input: Record<string, never> },
      )
      ...
      .map((command) => {
        ...
        return { Bucket: input.Bucket, Keys: input.Delete.Objects.map((object) => object.Key) };
      });
  }

  it("removes every key in one request against the write bucket", async () => {
  ...
  it("drops a key outside the app's namespace and still removes the rest", async () => {
    const { discardStoredObjects } = await import("./upload-actions");
    const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    ...
    expect(deleteBatches()).toEqual([
      { Bucket: "test-bucket", Keys: ["covers/chart-1/opt-a.webp"] },
    ]);
    consoleSpy.mockRestore();
  });
  ...
  it("logs the keys R2 reports it could not remove", async () => {
    ...
    expect(consoleSpy.mock.calls.flat().join(" ")).toContain("files/chart-1/pattern.pdf");
    consoleSpy.mockRestore();
  });
