 ...
 */
export function MainDashboard({ data, startNextCards, imageUrls }: MainDashboardProps) {
  return (
    <div className="mx-auto max-w-[1100px] space-y-16">
      {/* Page header + Quick Add */}
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-2xl font-semibold">Your Library</h1>
                ...
                <ScrollableRow>
                  {data.currentlyStitching.map((p) => (
                    <CurrentlyStitchingCard
                      key={p.projectId}
                      project={p}
                      imageUrl={imageUrls[p.coverThumbnailUrl ?? ""] ?? null}
                    />
                  ))}
                </ScrollableRow>
              ) : (
