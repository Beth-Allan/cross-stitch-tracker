...
| T-15-02   | Tampering              | mitigate    | CLOSED | `addChartFileSchema` (`validations/upload.ts`) validates every field, with `url` parsed by `parseStorageKey` and pinned to the `files` category; `getPresignedUploadUrl` applies `resolveChartFileContentType` before signing, so a chart file is signed only under the exact type that rule produces for its name (item P13b, 2026-08-19 — the old `ALLOWED_FILE_TYPES` is gone)                                                                                                                                   |
...
| T-15-06   | Tampering              | mitigate    | CLOSED | One rule, both sides (item P13b, 2026-08-19): `resolveChartFileContentType(fileName, declaredType)` in `validations/upload.ts` decides what a chart file may be and what it is stored as. Both clients call it before uploading; `getPresignedUploadUrl` and `addChartFile` each refuse anything that is not exactly what it would have produced — the latter against the type `HeadObject` reports, not the caller's claim. The two byte-identical allowlists that used to let browser and server disagree are one |
...
> **Done — item P13b, 2026-08-19.** The recommendation was taken, and the divergence this note predicted had already happened by another route: the two constants stayed identical, but the _client_ additionally accepted a file on its extension while the server checked only the type, so a Pattern Maker `.xsd` passed the browser and was refused at the presign step. There is now one list and one rule (`resolveChartFileContentType`), applied by both sides.
