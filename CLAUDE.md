...
- `npm run gate` = `prisma generate` → `format:check` → `lint` → `tsc --noEmit` → `test` →
  ...
  **`lint` is `--max-warnings 0`:** there is no passing tier below "error", so a new warning is a
  red gate. Two rule configs carry their reasoning in `eslint.config.mjs` — an `_` prefix means
  deliberately unused, and `no-img-element` is off (presigned URLs cannot be optimized).
