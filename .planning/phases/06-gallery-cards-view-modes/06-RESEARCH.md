...
1. **Kitting dot "not-applicable" heuristic**
   - What we know: No explicit "chart uses beads" flag exists on the schema
   - What's unclear: Whether the KITTED+ heuristic is always correct (user might be KITTED but just didn't add beads to tracker)
   - Recommendation: Ship with the heuristic (A2 above), add a backlog item for an explicit flag if users report confusion

2. **Cover image in gallery cards: presigned URL vs thumbnail**
   - What we know: Charts have both `coverImageUrl` (full) and `coverThumbnailUrl` (small). Gallery cards show large images.
   - What's unclear: Whether thumbnails (generated by Sharp) are high enough resolution for `aspect-[4/3]` gallery display
   - Recommendation: Use full `coverImageUrl` for gallery cards (visual impact), `coverThumbnailUrl` for list view small thumbs. Resolve both in server component.

3. **nuqs NuqsAdapter placement**
   - What we know: Needs to wrap the app in root layout
   - What's unclear: Whether it conflicts with existing providers (Sonner, auth session)
   - Recommendation: Add as outermost wrapper in `<body>` in `src/app/layout.tsx`. No known conflicts with Sonner or next-auth session. [CITED: https://nuqs.dev/docs/adapters]
