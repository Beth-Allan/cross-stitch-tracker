...
The DesignOS map: every screen the original whole-app design covered has a reference in
`product-plan/sections/`. **Read the design before building** — building UI from imagination is
the banned move (hard rule 4).

**Where this ranks.** DesignOS is **historical input, not the spec** (Beth's ruling D-05). Design
canon in `docs/design/screens/` supersedes it screen by screen as `/design-session` produces it;
this map is what you follow for screens canon has not reached yet. How to read a reference, and
how to adapt it to this stack rather than copying it, is `.claude/rules/ui-design-reference.md`.
...
## stitching-sessions-and-statistics
...
## fabric-series-and-reference-data
...
## gallery-cards-and-advanced-filtering
