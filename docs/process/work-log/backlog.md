...
(git history, commit `2860057`) and from `.planning/REQUIREMENTS.md`'s Future Requirements
...
| **Supply detail modal**                                     | read-only view of a supply with a "used in projects" list. Detail note survives at `.planning/backlog/999.1-supply-detail-modal.md` until step 5 archives it.                                                                                                                                                                                          | 999.1 / FEAT-F05    |
| **Bulk supply editor**                                      | edit many supplies at once instead of one at a time. Detail note at `.planning/backlog/999.2-bulk-supply-editor.md`.                                                                                                                                                                                                                                   | 999.2 / FEAT-F06    |
| **Fabric type hierarchy**                                   | replace the flat fabric-type dropdown with a hierarchy. **Partly a domain question** — what the hierarchy's levels are is Beth's vocabulary, so this needs a `/stitch-fact` pass before planning. Detail note at `.planning/backlog/999.3-fabric-type-hierarchy.md`.                                                                                   | 999.3 / FEAT-F07    |
...
| **Project supplies as a separate tab**                      | move supplies off the chart detail page into `/charts/[id]/supplies`. Written 2026-04-10 when the page was shorter; **the design track will answer this on its own** at the chart-detail redesign, so it is here as an input rather than a feature to plan separately. Detail note at `.planning/backlog/999.4-project-supplies-tab-layout.md`.        | 999.4               |
