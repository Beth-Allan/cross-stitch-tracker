...
| P10 dependency patch session                    | built  | 2026-08-17 | Opus 5 | all 29 npm advisories cleared (prod graph 2 critical + 13 high → 0); `@types/node` 20→22 folded in; `agentRules: false` per Beth's ruling; found that preview deployments cannot log in at all — new drift row + ledger row, owner R-1 |
